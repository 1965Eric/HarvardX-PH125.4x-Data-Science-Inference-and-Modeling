...
$Pr(Z \le .01/\sqrt {\bar{X}(1 - \bar{X})/N}) - Pr(Z \le -.01/\sqrt {\bar{X}(1 - \bar{X})/N})$
...
99% test accuracy when disease is absent:  $Pr(- \mid D = 0) = 0.99$ 
...
- [ ] B. There is no flaw. The multiplicative rule always applies in this way: $Pr(\text{A and B}) = Pr(A)Pr(B)$
- [ ] C. Sir Meadow should have added the probabilities: $Pr(\text{A and B}) = Pr(A) + Pr(B)$
...
- [X] C.  $\frac{Pr(\text{two children found dead with no evidence of harm} \mid \text{mother is a murderer})Pr(\text{mother is a murderer})}{Pr(\text{two children found dead with no evidence of harm})}$
- [ ] D. 1/8500
