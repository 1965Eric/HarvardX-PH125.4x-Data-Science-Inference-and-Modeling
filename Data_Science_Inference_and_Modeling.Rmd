...
$Y_ij = d + b_i + \epsilon_ij$

with $i = 1, 2$ indexing the two pollsters, $b_i$ the bias for pollster $i$, and $\epsilon_ij$ poll to poll chance variability. We assume the $\epsilon$ are independent from each other, have expected value ```0``` and standard deviation $\sigma_i$ regardless of $j$.

Which of the following statements best reflects what we need to know to determine if our data fit the urn model?

- [ ] A. Is $\epsilon_ij = 0$?
- [ ] B. How close are $Y_ij$ to $d$?
- [X] C. Is $b_1 \neq b_2$?
...
On the right side of this model, only $\epsilon_ij$ is a random variable. The other two values are constants.

What is the expected value of $Y_1j$?
